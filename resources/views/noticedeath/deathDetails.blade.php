          ...
          <div class="row p-t-20">
            <div class="col-md-4">
              <div class="form-group">
                  ...
                  <input type="date" id="dodeath" name="dodeath" onchange="checkaccddate()"  value="{{substr($deathdetail->dodeath,0,4)}}-{{substr($deathdetail->dodeath,4,2)}}-{{substr($deathdetail->dodeath,6,2)}}" class="form-control" >
                  @else
                  <input type="date" id="dodeath" name="dodeath" value="" class="form-control" onchange="checkaccddate()">
                  @endif
                  <label class="control-label" id='lbldeatherror' style='color:red'></label>
              </div>
            </div>
            <div class="col-md-4">
              <div class="form-group">
            ...
            <div class="row">
              <div class="col-md-12 col-lg-4">
                <div class="form-group">
                    ...
                    @if (!empty($accinfo))
                    <input type="date" class="form-control" id="accddate" name="accddate" onchange="checkaccddate()" value="{{substr($accinfo->accddate,0,4)}}-{{substr($accinfo->accddate,4,2)}}-{{substr($accinfo->accddate,6,2)}}">
                    @else
                    @if (Session::get('accddate'))
                    <input type="date" class="form-control" id="accddate" name="accddate" onchange="checkaccddate()" value="{{substr(Session::get('accddate'),0,4)}}-{{substr(Session::get('accddate'),4,2)}}-{{substr(Session::get('accddate'),6,2)}}">
                    @else
                    <input type="date" class="form-control" id="accddate" name="accddate" value="" onchange="checkaccddate()">
                    @endif
                    @endif
                    <label class="control-label" id='lblaccderror' style='color:red'></label>
                </div>
...
<script>
  ...
  function checkaccddate()
  {
     
        var accddate = document.getElementById('accddate').value;
      var deathdate = document.getElementById('dodeath').value;
      var lblaccderror = document.getElementById('lblaccderror');
      var lbldeatherror = document.getElementById('lbldeatherror');
    ...
    var days2 = getdiff(currdate,deathdate);
      
      var days = getdiff(deathdate,accddate);
      
      
    if(days > 1)
    {
        lblaccderror.innerHTML = 'Accident date must not after death date';
        btnsubmit.disabled = true;
        return;
    }
    else
    {
        lblaccderror.innerHTML = '';
        btnsubmit.disabled = false;
    }
    
    if(days2 > 1)
    {
        lbldeatherror.innerHTML = 'Death date must not after current date';
        btnsubmit.disabled = true;
        return;
    }
    else
    {
        lbldeatherror.innerHTML = '';
        btnsubmit.disabled = false;
    }
  }
...
$(document).ready(function() {
  ...
  function counttotalwork(mcid,workid){
    ...
    if(startdate != "" && enddate != ""){
      ...
      if (totalwork <= 0){
        total.value = "";
        lblmcerror.innerHTML  = 'Work end date must not before start date';
        btnsubmit.disabled = true;
      }
      else{
